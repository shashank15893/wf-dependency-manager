...
// Execution Info Component
const ExecutionInfo = ({ selectedNode }) => {
  ...
  return (
    <div className="bg-white p-4 border rounded-md shadow-sm text-sm">
      <div className="grid grid-cols-2 gap-x-4 gap-y-2">
        ...
        <div>{selectedNode.EXECUTION_DT ? formatDate(selectedNode.EXECUTION_DT) : 'N/A'}</div>
        
        <div className="font-medium">SLA:</div>
        <div>{selectedNode.SLA || 'N/A'}</div>
        
        <div className="font-medium">Type:</div>
        <div>{selectedNode.NODE_DESCRIPTION || 'N/A'}</div>
        
        <div className="font-medium">Status:</div>
        <div className="font-medium">
          {selectedNode.STATUS ? (
            <span className={`px-2 py-1 rounded text-sm ${
              selectedNode.STATUS.toLowerCase() === 'failed' ? 'bg-red-100 text-red-800' :
              selectedNode.STATUS.toLowerCase() === 'completed' ? 'bg-green-100 text-green-800' :
              selectedNode.STATUS.toLowerCase() === 'running' ? 'bg-blue-100 text-blue-800' :
...
const SupplyChainDependencyChart = () => {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {selectedNode && (
        <div className="mb-4">
          <h2 className="text-lg font-semibold mb-2">Selected Node: {selectedNode.NODE_NM}</h2>
          <ExecutionInfo selectedNode={selectedNode} />
        </div>
      )}
